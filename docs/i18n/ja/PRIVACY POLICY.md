...
製品体験の向上を支援するために、「ソフトウェア」は**匿名の使用統計データ**を送信できます。この機能は完全にオプションであり、**既定では無効**です。明示的に有効にしない限り、何も送信されません。
...
**有効にする方法:**
匿名の利用統計はオプトインです。環境変数で `EXPERIENCE_IMPROVEMENT=true` を設定した場合のみ送信されます。未設定（または `false`）の場合、データは送信されません。
