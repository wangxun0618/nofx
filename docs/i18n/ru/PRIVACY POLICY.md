...
Чтобы помочь нам улучшить продукт, «Программное обеспечение» может отправлять **анонимную статистику использования**. Эта функция полностью опциональна и **по умолчанию отключена**: данные не отправляются, пока вы явно её не включите.
...
**Как включить:**
Это добровольное согласие. Установите `EXPERIENCE_IMPROVEMENT=true` в переменных окружения, чтобы включить сбор; если переменная не задана (или равна `false`), данные не отправляются.
